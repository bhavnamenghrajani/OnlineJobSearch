...
public class RegisterUserController {
	...
	public String updateProfile(@ModelAttribute("user") User user, @RequestParam("ID") String userID) {
		...
		userDao.updatePersonalInformation(userID, user.getAddressLine1(), user.getAddressLine2(), user.getCity(),
				user.getCountry(), user.getCountryState(), user.getEmailAddress(), user.getHomeCountryCode(),
				user.getHomePhone(), user.getOfficeCountryCode(), user.getOfficePhone(), user.getZipCode());
		return "JobSeekerMain";
	}

	@RequestMapping(value = "/login.htm", method = RequestMethod.POST)
	protected String authenticate(@ModelAttribute("user") User user, BindingResult result, HttpServletRequest request)
			throws Exception {
